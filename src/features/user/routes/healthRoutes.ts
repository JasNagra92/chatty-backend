...
class HealthRoutes {
  ...
  public health(): Router {
    this.router.get('/health', (req: Request, res: Response) => {
      res
        ...
        .send(`Health: Server instance is healthy with process id ${process.pid} on ${moment().format('LL')}, launched with code deploy`);
    });
  ...
  public fiboRoute(): Router {
    this.router.get('/fibo/:num', async (req: Request, res: Response) => {
      ...
      //   url: config.EC2_URL
      // });
      res.status(HTTP_STATUS.OK).send(`Fibonacci series of ${num} is ${result} and it took ${end - start}ms
      with process id ${process.pid} on ${moment().format('LL')}`);
    });
